...
:name(_name), signe(false), grade_sign(_grade_sign), grade_exec(_grade_exec)
{
  try
  {
    if (grade_sign > 150)
...
void Form::GradeTooHighException::detail() const
{
  std::cout << "Grade too high exception" << std::endl;
}

void Form::GradeTooLowException::detail() const
{
  std::cout << "Grade too low exception" << std::endl;
}

void Form::UnsignedException::detail() const
{
  std::cout << "Not able to execute because form has not been signed" << std::endl;
}
